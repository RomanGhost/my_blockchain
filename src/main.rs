...
Mutex}; use tokio::io::{AsyncBufReadExt, AsyncWriteExt, 
BufReader}; use tokio::net::TcpListener; use 
...
async fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        let (socket, _) = listener.accept().await?; let tx 
        = tx.clone(); let mut rx = tx.subscribe(); let 
        clients = Arc::clone(&clients); tokio::spawn(async 
        move {
            let (reader, mut writer) = 
            socket.into_split(); let mut reader = 
            BufReader::new(reader); let mut nickname = 
            String::new();
            // Запрос ника у пользователя
            writer.write_all(b"Enter your nickname: 
            ").await.unwrap(); reader.read_line(&mut 
            nickname).await.unwrap(); let nickname = 
            ...
            loop { let mut message = String::new(); 
                tokio::select! {
                    // Чтение сообщения от клиента
                    result = reader.read_line(&mut 
                    message) => {
                        ...
                        tx.send(format!("{}: {}", 
                        nickname, message)).unwrap();
                    }
                    // Получение сообщения из канала и 
                    // отправка клиенту
                    result = rx.recv() => { let message = 
                        result.unwrap(); 
            ...
            tx.send(format!("{} has left the chat", 
            nickname)).unwrap(); let mut clients = 
            clients.lock().unwrap(); 
